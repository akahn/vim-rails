...
function! rails#buffer()
  if exists('b:rails_root')
    return extend(extend({'_app': rails#app(), '_number': bufnr('%')},s:buffer_prototype,'keep'),s:readable_prototype,'keep')
  else
    return {}
  endif
endfunction
...
call s:add_methods('buffer',['path','name','lines','getline','type_name'])
